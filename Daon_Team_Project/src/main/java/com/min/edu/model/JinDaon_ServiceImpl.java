...
public class JinDaon_ServiceImpl implements JinDaon_IService {
	...
	public boolean deleteCourse(String cou_code) {
		logger.info("deleteCourse과정삭제,{}", cou_code);
		return dao.deleteCourse(cou_code);
	}

	// 예정 과정 수정
	@Override
	public boolean wupdateCourse(Course_Dto dto) {
		logger.info("wupdateCourse과정수정",dto);
		return dao.wupdateCourse(dto);
	}
...
//과목 삭제
	...
	public boolean deleteSubject(String sub_code) {
		logger.info("deleteSubject과목삭제", sub_code);
		return dao.deleteSubject(sub_code);
	}
